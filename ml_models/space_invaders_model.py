...
def create_SpaceInvaders_lstm(env_shape, **kwargs):
    # Define layers
    input_layer = Input(shape=env_shape)
    c1 = Conv2D(128, (11,11), strides=(4,4), 
                            padding='same', 
                            activation='relu')(input_layer)
    mp1 = MaxPooling2D(pool_size=(3,3), strides=(2,2))(c1)

    c2 = Conv2D(512, (5,5), strides=(2,2), 
                            padding='same', 
                            activation='relu')(mp1)
    mp2 = MaxPooling2D(pool_size=(3,3), strides=(2,2))(c2)

    c3 = Conv2D(256, (3,3), strides=(1,1), 
                            padding='same', 
                            activation='relu')(mp2)

    c4 = Conv2D(512, (3,3), strides=(1,1), 
                            padding='same', 
                            activation='relu')(c3)
    
    c5 = Conv2D(256, (3,3), strides=(1,1), 
                            padding='same', 
                            activation='relu')(c4)
    mp3 = MaxPooling2D(pool_size=(3,3), strides=(2,2))(c5)
    
    fl = Flatten()
    td = TimeDistributed(fl)(mp3)
    #d1 = Dense(64, activation='relu')(fl)
    ...
    lstm5 = LSTM(256)(lstm4)
    # output_layer = Dense(action_shape, activation='linear')(lstm5)

    model = Model(input_layer, lstm5, name=__name__)
